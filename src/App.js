...
function App() {
  ...
  let getData = (page=10)=>{
    try {
      fetch(`${config.base_url}/api/students?pagination[start]=0&pagination[limit]=${page}`).then((res)=>{
        return res.json();
      }).then((data)=>{
  ...
  return (
    <>
    <h1 className="d-flex justify-content-center">Read opration Load More</h1>
    <Button className="d-flex justify-content-center" variant="outline-dark" onClick={()=>{getData()}}>get data</Button>
      {
        data.data.length > 0 &&
        <>
      ...
      <div className="text-center" onClick={()=>{}}>
         <button id="a1" variant="primary" onClick={() => {lode()}} >Load more</button>
      </div>
      {
